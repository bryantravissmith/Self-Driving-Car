...
def stays_on_road_cost(traj, target_vehicle, delta, T, predictions):
    s, d, t = traj
    logitudial_eq = to_equation(d)
    all_logitudial = [logitudial_eq(float(T)/100 * i) for i in range(100)]
    max_logitudial = max(all_logitudial, key=abs)
    min_logitudial = min(all_logitudial, key=abs)
    if max_logitudial > 4 or min_logitudial < 0:
        return 1.0
    else:
        return 0
...
def exceeds_speed_limit_cost(traj, target_vehicle, delta, T, predictions):
    s, d, t = traj
    velocity_eq = to_equation(differentiate(s))
    all_velocity = [velocity_eq(float(T)/100 * i) for i in range(100)]
    max_velocity = max(all_velocity, key=abs)
